...
# ///
# Deterministic golden-fixture generator (normative since wayfinder ticket 05):
# builds the skeleton-chain store documents, computes their real CIDv1
# addresses, and writes <name>.dagjson.json + CIDS.txt. Uses the hashberg
# dag-cbor/multiformats pair on purpose — independent of the production
# libipld encoder, so the fixtures stay a cross-implementation check.
# CI regenerates and diffs against the committed files. Re-run with:
...
def cid_for(doc) -> CID:
    return CID(
        "base32", 1, "dag-cbor", multihash.digest(dag_cbor.encode(doc), "sha2-256")
    )
...
put(
    "claim-values-apps",
    {"schema": 1, "kind": "values", "key": "apps", "content": values_blob},
)
put(
    "claim-secret-proxmox",
    {
        "schema": 1,
        "kind": "secret",
        "shape": "value",
...
put(
    "derivation-values-apps",
    {  # built-in: freckles version, no plugin CID
        "schema": 1,
...
for name, doc in docs.items():
    ...
    manifest.append(
        f"{cids[name]}  {name}  ({len(dag_cbor.encode(doc))} bytes dag-cbor)"
    )
manifest += [
    f"{c}  blob:{n}"
    for n, c in [
