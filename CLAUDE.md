...
- project_name: `freckles`
- project_description: Turns minimal declarative configuration into running infrastructure and IT state through a DAG of content-addressed operations.
- python base module: `freckles`
...
### Package Structure
- Source code: `src/freckles/` (namespace package)
- Tests: `tests/`
...
### Debug Utilities
Importing `freckles._debug` installs several debug helpers as builtins (when the
dev dependencies are available). `tests/conftest.py` imports it, so they are always usable in
tests; in a script or REPL, run `import freckles._debug` first. The package
...
None on purpose: the test suite is one undivided whole (no fast/slow split; see
the Testing strategy doc in `.scratch/impl/assets/10-testing-strategy.md`).
`--strict-markers` makes accidental marker use fail loudly.

## Canonical Project Documents

- `CONTEXT.md` — the canonical glossary; design discussions and documents use
  its vocabulary (and avoid its retired terms).
- `docs/design.md` — the design; `docs/vision.md` is background.
- `docs/adr/` — binding architecture decisions (ADRs 0001–0006).
- `conformance/` — normative wire/store schemas and golden CID fixtures;
  data-only and language-agnostic, regenerated by `conformance/golden/make_golden.py`.
- `.scratch/impl/map.md` — the wayfinder map driving the v1 implementation
  effort (tickets under `.scratch/impl/issues/`); `.scratch/design/` is the
  completed design effort's record.
